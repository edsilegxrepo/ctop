...
func (dc *Docker) ReadDir(dirPath string) ([]models.FileInfo, error) {
	...
	for {
		...
		modeStr := os.FileMode(header.Mode).String()
...
func (dc *Docker) Upload(srcHostPath, dstContainerPath string) error {
	...
	srcFile, err := os.Open(srcHostPath)
	if err != nil {
		return err
	}
	defer srcFile.Close()
	...
	if srcStat.IsDir() {
		baseDir := filepath.Dir(srcHostPath)
		err = filepath.Walk(srcHostPath, func(curPath string, info os.FileInfo, walkErr error) error {
			if walkErr != nil {
				return walkErr
			}
