...
- **Docker Context & Rootless Socket Resolution (`pkg/connector/docker_context.go`):** Automatically detects and parses Docker CLI contexts from `DOCKER_HOST`, `DOCKER_CONTEXT`, `~/.docker/config.json` (`currentContext`), and `~/.docker/contexts/meta/<sha256>/meta.json` to seamlessly connect to Colima, Rancher Desktop, or Docker Desktop sockets. When standard `/var/run/docker.sock` is absent on Linux, it automatically discovers and connects to rootless Docker sockets (`$XDG_RUNTIME_DIR/docker.sock` or `/run/user/<uid>/docker.sock`).
...
- **`Collector` (`pkg/connector/collector/`):** Responsible for collecting metrics for a single container (`docker.go`, `docker_logs.go`, `runc.go`, `mock.go`). Collectors run in dedicated goroutines per container, streaming `models.Metrics` and `models.Log` structures over channels. Supports cgroups v2 `io.stat` fallback and real-time throughput rate calculations. The Docker log collector (`docker_logs.go`) auto-detects TTY mode to bypass multiplexed stream headers, dynamically recovers via raw terminal fallback when stream headers are desynchronized, strips trailing CRLF, resolves inline progress overwrites (`\r`), and sanitizes null bytes.
...
- **`internal/cwidgets/`:** Contains custom reusable UI components: compact grid rows (`internal/cwidgets/compact/`) and detailed single-container views with sparklines and history ring buffers (`internal/cwidgets/single/`). The TUI `Logs` widget maintains an in-place rotated buffer capped at 8,192 lines with zero heap allocations at saturation (`copy(w.Entries, w.Entries[1:])`), live counter awareness `(current/8192)`, runtime line wrapping (`w`), and dual-mode inclusive/exclusive filtering (`x` or `<Tab>` in filter prompt). The Process & Env inspector (`Env`) enforces case-insensitive alphabetical sorting of environment variables while preserving sensitive secret masking (`[u]`).
...
- **Embedded SPA Dashboard (`pkg/web/dashboard.html`):** Single-page web application embedded via `//go:embed`. Features dark glassmorphism styling, live container metrics overview, search filtering, rate mode toggle, real-time HTML5 Canvas 2D telemetry sparklines, downloadable pretty-formatted JSON export buttons, plain-text interactive Report Viewer modal (one-click copy to clipboard, download .txt), and a per-container drill-down inspection modal with live CPU/memory/network/disk graphs, a 5-row running metric history table, and 7 inspect tabs (`[o] overview & metrics`, `[l] logs`, `[v] volumes & mounts`, `[n] networking & ports`, `[E] process & env`, `[P] in-container top`, `[w] web & probes`). The logs module implements `requestAnimationFrame` + `DocumentFragment` render batching to maintain 60 FPS during log floods, quick log-level filtering pills (`[ALL]`, `[ERROR]`, `[WARN]`, `[INFO]`), dual-mode export (`Active Buffer` & `Full History from Docker Engine`), 6,000 DOM line limit awareness, and an `[Include/Exclude]` filter mode toggle. The process & env inspector features case-insensitive alphabetical sorting, a `[Clear]` filter action, and an `[Include/Exclude]` filter toggle. The frontend actively monitors operator interaction (`mousemove`, `keydown`, `click`, `scroll`, `touchstart`) against backend `session_timeout` and presents an auto-logout modal on inactivity.
- **Read-Only REST API, Session Store & Reverse Proxy Routing (`pkg/web/server.go`):** Exposes `/api/v1/health`, `/api/v1/metrics`, `/api/v1/containers`, `/api/v1/containers/{id}`, `/api/v1/containers/{id}/logs` (supporting SSE `?stream=true`, full history download `?download=true&tail=all`, and snapshot JSON), `/api/v1/containers/{id}/top`, and `/api/v1/export` (supporting cluster and `?container=<id>` pretty JSON downloads). Employs a thread-safe, memory-bounded `SessionStore` (capacity 100, LRU eviction, 24h absolute TTL) with a configurable sliding-window idle timeout (default 30m / 1800s via `--session-timeout`, 0 to disable) and active SSE keepalive session expiration that terminates idle streaming connections. Supports subpath prefixing via `--url-prefix <subpath>` (e.g. `/probe`) for seamless NGINX, Caddy, or Traefik reverse proxy integration. Strictly enforces read-only access (rejects `POST`/`PUT`/`DELETE` with 405 Method Not Allowed; exposes zero Docker mutator or exec commands).
- **Headless Daemon Mode (`--headless`):** Allows running `ctop` purely as a background telemetry server (e.g. within Docker or systemd via `ctop service <action>`) without initializing terminal UI libraries. Traps `SIGINT`/`SIGTERM` for graceful shutdown and strictly enforces that `--web-auth-token` requires TLS for any remote access, with plain HTTP restricted exclusively to local loopback (`127.0.0.1`). Configurable via `--session-timeout` to tailor session lifetimes for human operators or disable timeouts (`--session-timeout 0`) for continuous NOC wall-boards and kiosks.
...
```text
ctop/
├── main.go                       # CLI entry point: flag parsing (--rate, --cumulative, --tls-*, --session-timeout, etc.), theme init, main loop
...
│   │   ├── server.go             # Read-only HTTP/REST endpoint multiplexer, rate limiter, and sliding session manager
...
│   │   └── dashboard.html        # Embedded zero-dependency HTML5 Canvas 2D telemetry dashboard (filters, logs, session guard)
│   │
