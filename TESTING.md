...
23. **`pkg/connector/collector/docker_logs.go` (TTY Multiplexing & Progress Overwrites)**: Containers with TTY enabled do not emit 8-byte multiplexed stream headers, causing demux failures. Resolved by inspecting `Tty` mode, setting `RawTerminal: true`, adding dynamic retry fallback, and stripping carriage return overwrites (`\r`).
24. **`pkg/connector/docker_context.go` (Rootless Docker Socket Auto-Discovery)**: Rootless Docker stores daemon sockets in `$XDG_RUNTIME_DIR/docker.sock` or `/run/user/<uid>/docker.sock`. Resolved by auto-discovering rootless sockets on Linux when `/var/run/docker.sock` is absent.
25. **`pkg/web/server.go` & `dashboard.html` (Full-Height Modal Viewports, NDJSON Export & Comprehensive Telemetry Snapshots)**: Removed hardcoded tab height caps across container inspection panes, implemented sticky headers with dynamic full-viewport scrolling, added container name prefixing to full history downloads, added NDJSON export for active log buffers, and enriched both text reports and JSON snapshots with health status, formatted uptime, memory RSS/cache, cumulative I/O bytes, process tables, network probes, filesystem diffs, and generated commands.
26. **`pkg/web/server.go`, `web_bridge.go`, `main.go` & `dashboard.html` (Session Idle Timeout & Inclusive/Exclusive Filters)**: Implemented 30-minute session idle timeout (default 1800s, configurable via `--session-timeout`), sliding-window activity tracking, SSE stream disconnect upon idle expiry, alphabetical sorting of environment variables, [Clear] filter button, and [Include/Exclude] filter toggles across environment variables and container log viewer.
27. **`menus.go`, `internal/theme/theme.go` & `pkg/connector/manager/docker.go` (Interactive Shell Execution, Host Terminal Raw Mode, Keypad Pollution Isolation, Termbox Suspension, Unprivileged Accounts, TTY Demux, Window Resize & Event Draining)**:
    - *Root Cause 1*: `ExecShell` was executing `/bin/sh -c "printf '...' && eval \`grep ^$(id -un): /etc/passwd | cut -d : -f 7-\`"`. For containers running as downgraded unprivileged service accounts (such as `mysql`, `postgres`, `redis`), `/etc/passwd` lists `/sbin/nologin` or `/bin/false`. Executing `eval` invoked `/sbin/nologin`, producing `"This account is not available"` and immediately exiting.
    - *Root Cause 2*: `Docker.Exec` configured `StartExecOptions` with `&frameWriter{os.Stdout, os.Stderr, os.Stdin}` on an interactive execution (`Tty: true`). Because Docker TTY streams are raw and unmultiplexed, `frameWriter` parsed raw terminal escape sequences as 8-byte frame headers, returning `errWrongFrameFormat` and aborting output.
    - *Root Cause 3*: In `go-dockerclient`'s `hijack` handler, `StartExec` launches a background goroutine running `io.Copy(rwc, hijackOptions.in)` to pump standard input to the container. When the container shell terminated, `noClosableReader` prevented `os.Stdin` from being closed, leaving `io.Copy` blocked on a pending `os.Stdin.Read`. `StartExec` could not return until this input goroutine exited. Consequently, the user's very first keypress after exiting the container was consumed by this lingering `os.Stdin.Read` call rather than `termbox`. Only after reading that key did `io.Copy` attempt a write to the closed Docker socket, fail with broken pipe, and allow `StartExec` to return to `ctop`, requiring a second keypress from the user.
    - *Root Cause 4*: Unlike the official Docker CLI (`docker exec -it`), which explicitly calls `term.SetRawTerminal(os.Stdin.Fd())`, `go-dockerclient`'s `RawTerminal: true` flag only controls whether streams are demuxed with `stdcopy.StdCopy` or copied with `io.Copy`. It NEVER modified host terminal attributes. When `ctop` called `tb.Close()`, termbox restored `orig_tios` (cooked/canonical mode with local echo enabled). In cooked mode with local echo:
      - When container shells (such as BusyBox `ash` in Alpine, `authelia`, or `traefik`) queried cursor position via DSR (`\033[6n`), the host terminal driver generated the Cursor Position Report (`\033[44;8R` / `\033[44;5R`) and immediately ECHOED it to the screen as `^[[44;8R` / `^[[44;5R`. Furthermore, because canonical mode buffers input until `\n`, the CPR response was not delivered to the container shell in time, leaking into the prompt.
    - *Root Cause 5*: While `CreateExecOptions` had `Tty: true`, `StartExecOptions` in `Docker.Exec` omitted `Tty: true`. In `go-dockerclient`, `StartExecOptions` has an independent `Tty: bool` field sent in the POST body to `/exec/{id}/start`. Without `Tty: true`, the Docker daemon treated the execution stream as non-TTY and prepended standard 8-byte multiplex headers (`[stream_id, 0, 0, 0, size_b1, size_b2, size_b3, size_b4]`) to every stdout payload packet. The 8th byte (`size_b4`) represented the packet payload size (e.g. `0xB3` = 179 bytes for `/etc/hosts`, `0x3D` = 61 bytes, `0x4A` = 74 bytes). In terminal emulators, bytes 0-6 (`\x01\x00\x00\x00\x00\x00\x00`) are non-printing control characters, while byte 7 was displayed as the leading character on the first line: `0xB3` rendered as `` (invalid UTF-8), `0x3D` rendered as `=`, and `0x4A` rendered as `J`.
    - *Resolution*:
      1. Explicitly placed `os.Stdin` into true raw mode during container exec via `term.SetRawTerminal(inFd)` (from `github.com/moby/term`, matching Docker CLI behavior), and restored it upon completion with `defer term.RestoreTerminal(inFd, oldState)`. In raw mode, host local echo is disabled, line buffering is disabled, CPR responses are silently passed to the container shell without leaking onto the screen, and character pollution is eliminated.
      2. Cleaned host terminal screen (`\033[H\033[2J`) before entering the container shell so the session begins at row 1, column 1 on a pristine screen.
      3. Suspended `termbox` cleanly before exec via `tb.Close()` and restored it after exec via `tb.Init()`, `tb.SetInputMode(tb.InputEsc)`, `tb.HideCursor()`, `_ = tb.Sync()`, and `RedrawRows(true)`.
      4. Removed `printf` and `clear` injection wrappers, invoking a clean interactive shell (`if [ -x /bin/bash ]; then exec /bin/bash; elif command -v bash >/dev/null 2>&1; then exec bash; else exec sh; fi`) with clean fallback binaries.
      5. Synchronized container PTY dimensions using `term.GetWinsize(outFd)` and `ResizeExecTTY(execCmd.ID, termH, termW)`.
      6. Retained `cancellableStdin` (with `unix.Poll` and self-pipe) so container exit unblocks cleanly with `io.EOF` without hanging or stealing user keypresses.
      7. Flushed host terminal input queue (`flushTerminalInput(inFd)` via `unix.TCIFLUSH` and non-blocking poll drain on Unix, and `windows.FlushConsoleInputBuffer` on Windows) immediately upon switching to raw terminal mode.
      8. Configured `Tty: true` in `api.StartExecOptions` in `Docker.Exec` so that Docker daemon attaches in true TTY mode without prepending 8-byte multiplexed stream headers, completely eliminating the first character pollution (``, `=`, `J`) on command outputs.
28. **`internal/cwidgets/single/env.go`, `internal/cwidgets/single/logs.go`, `grid.go` & `menus.go` (Text UI Env Alpha Sorting & Logs Inclusive/Exclusive Filter)**:
    - *Env Alpha Sorting*: Alphabetically sorted container environment variables case-insensitively by key (`sort.Slice` with `strings.ToLower` comparison) in `rebuild()`, preserving sensitive secret masking (`[masked]`).
    - *Main Grid [E] Shortcut*: Bound `E` key on the main container grid to jump directly to `5: Env/Proc` (`SingleViewProcess`), mirroring `l` jumping to `2: Logs`. Updated help modal dialog (`helpDialog`).
    - *Logs Include/Exclude Filter Mode*: Added `FilterExclude bool` to `Logs` widget (defaulting to `false` for inclusive filtering). Added `ToggleFilterMode()`, `SetFilterExclude()`, and `IsFilterExclude()`. Inverted filter matching when `FilterExclude` is active (`matches = !matches`). Updated the log viewer title bar to indicate mode (`[/filter: <query> (include)]` vs `[/filter: <query> (exclude)]`) and added `x: incl/excl` hotkey hint.
    - *Inline Filter Mode Toggle*: Added `<Tab>` / `<BackTab>` hotkey handling within the `/` filter input dialog to dynamically toggle between `[include mode]` and `[exclude mode]` while typing, and `x` hotkey in `TabLogs` to toggle mode on the fly.
...
| `TestDockerContextRootlessResolution` | Tests auto-discovery of Linux Rootless Docker sockets (`$XDG_RUNTIME_DIR/docker.sock` and `/run/user/<uid>/docker.sock`) when `/var/run/docker.sock` is absent | **PASS**: Resolves rootless user socket path without requiring manual `DOCKER_HOST` configuration |
...
| `TestNewDockerClientDefaultEndpoint` | Tests client creation when empty endpoint string is supplied, verifying fallback to default environment configuration | **PASS**: Successfully creates Docker client instance from standard default environment |
...
| `TestConnectorFormattingHelpers` | Tests string and metadata formatting helpers including `mountsFormat`, `apiPortsFormat` (instant container list port parsing), `containerPortsFormat` (active runtime ports with fallback to `HostConfig.PortBindings` and `Config.ExposedPorts` for stopped containers), and `containerWebPort` | **PASS**: Correctly formats volumes, instant API ports, stopped container published bindings, and web port URLs |
...
| `TestDockerLogsStreamWithTTYMockServer` | Tests streaming logs from containers configured with `Tty: true`, verifying raw stream consumption without multiplex headers and `\r\n` handling | **PASS**: Correctly ingests raw TTY container streams without multiplex demux errors |
| `TestDockerLogsStreamFallback` | Tests automatic retry fallback when container stream returns unrecognized header errors, dynamically switching to `RawTerminal: true` | **PASS**: Recovers seamlessly and resumes log stream on raw fallback |
| `TestDockerLogsSanitizeLine` | Tests line sanitization: Docker header stripping, trailing CRLF trimming, carriage return (`\r`) progress overwrite resolution, and null byte replacement | **PASS**: Resolves inline progress overwrites to latest segment and cleans null bytes |
| `TestDockerLogsNilClientSafety` | Tests safe behavior and clean channel closure when `DockerLogs` is initialized with a nil Docker client | **PASS**: Closes log channel safely without nil pointer dereference panic |
...
| `TestDockerManagerExecTtyAndStream` | Tests `Docker.Exec` against mock HTTP Docker daemon endpoints, verifying `Tty: true` on both `CreateExec` and `StartExec` requests to prevent 8-byte multiplex stream header corruptions, stdio stream attachment, raw terminal byte passthrough, and nil/error client guards | **PASS**: Configures `Tty: true` on Create and Start, dispatches raw streams without frame corruption, and safely handles errors |
| `TestCancellableStdinUnblocksOnClose` | Tests self-pipe non-blocking cancellation on `cancellableStdin`, ensuring that calling `Close()` unblocks active terminal `Read()` with `io.EOF` without requiring or stealing user keystrokes | **PASS**: `Read()` unblocks immediately with `io.EOF` upon `Close()` |
| `TestDockerManagerExecResizeTTY` | Tests container PTY window dimension synchronization during Exec, ensuring ResizeExecTTY is invoked with active terminal rows and columns | **PASS**: Invokes ResizeExecTTY with matching terminal height and width |
| `TestFlushTerminalInput` | Tests host terminal input buffer purging (`flushTerminalInput`), validating non-blocking draining of unread bytes on active pipes and safe return without panic on invalid descriptors | **PASS**: Safely drains pending input without hanging and handles invalid descriptors gracefully |
| `TestCancellableStdinNormalReadAndIdempotentClose` | Tests `cancellableStdin` stream reading with active payload, multiple idempotent `Close()` invocations, and immediate `io.EOF` delivery on subsequent reads | **PASS**: Reads data accurately, closes idempotently without panics, and returns immediate EOF |
...
| `TestLogLinesMaxCapacityInPlaceRotation` | Tests log capacity capping strictly at 8,192 entries when ingesting 8,292 lines, oldest 100 evicted, in-place slice rotation with zero heap reallocation, and title limit counter awareness `(8192/8192)` (`internal/cwidgets/single`) | **PASS**: Evicts oldest entries while retaining exactly 8,192 lines; displays upper limit counter |
| `TestLogsWidgetFilterAndSave` | Tests log level substring filtering and log export to disk with container name formatting (`internal/cwidgets/single`) | **PASS**: Filters lines matching query and writes log snapshot cleanly to disk |
| `TestLogsWidgetIncludeExcludeFilter` | Tests log Include vs Exclude filter modes (`FilterExclude`), `ToggleFilterMode()`, `SetFilterExclude()`, title mode indicator `[/filter: <query> (include)]` vs `[/filter: <query> (exclude)]`, line exclusion, empty query handling, and empty-state messaging (`internal/cwidgets/single`) | **PASS**: Default inclusive mode correctly shows matches; exclusive mode hides matches; empty query preserves full buffer; title accurately reflects mode |
| `TestEnvAlphaSorting` | Tests case-insensitive alphabetical sorting of container environment variables by key in `Env.rebuild()`, preserving sensitive value masking (`[masked]`) across `ToggleMask()` toggles (`internal/cwidgets/single`) | **PASS**: Unordered environment variables are sorted alphabetically by key; sensitive values remain masked |
...
| `TestTermDimensionsFallback` | Tests atomic terminal dimension fallback resolution (`TermDimensions`), verifying thread-safe cached dimensions when termbox is uninitialized/suspended, custom dimension propagation, and default 80x24 resets | **PASS**: Correctly returns atomic fallback dimensions without zero-dimension drops or race conditions |
...
| `TestMaskSecretAndEnv` | Tests secret value masking with `•••••••••••• [masked]` and environment variable obfuscation (`pkg/sanitize`) | **PASS**: Masks sensitive keys while preserving non-sensitive environment variables |
| `TestSanitizeCommandString` | Tests automated masking of sensitive keys within generated `docker run` commands and Compose YAML (`pkg/sanitize`) | **PASS**: Obfuscates passwords, tokens, and credentials in multiline commands |
...
| `TestGenerateWithSensitiveSecrets` | Tests credential masking in generated CLI run commands and Docker Compose service specifications (`pkg/generator`) | **PASS**: Sensitive secrets are masked with `•••••••••••• [masked]` and never output in cleartext |
...
| `TestWebServerContainerLogs` | Tests `/api/v1/containers/{id}/logs` endpoint across snapshot JSON, plain text download, SSE live streaming (`?stream=true`), missing container 404, and unsupported provider 501 (`pkg/web`) | **PASS**: Streams SSE events, serves log download files, and returns 404/501 error responses cleanly |
| `TestWebServerContainerLogsAdvanced` | Tests custom numeric tail limits (`tail=2`) and case-insensitive unbounded export (`tail=ALL`) streaming directly from Docker engine (`pkg/web`) | **PASS**: Applies custom tail limits and streams unbounded historical container logs |
| `TestWebDashboardLogViewerAssets` | Asserts embedded SPA dashboard assets (`dashboard.html`) serve `requestAnimationFrame` + `DocumentFragment` log batching, dual export options (`Active Buffer` & `Full History`), quick filter pills (`[ALL]`, `[ERROR]`, etc.), and 6,000 DOM line limit awareness (`pkg/web`) | **PASS**: Embedded web dashboard contains all log viewer performance, filtering, and export capabilities |
| `TestWebServerLogDownloadFilenameWithContainerName` | Tests server-side formatting of full history log download filename prefixed with container name (`ctop_logs_<name>_<id>_<timestamp>.log`), validating explicit `?name=` query param handling and fallback container snapshot lookup (`pkg/web`) | **PASS**: Content-Disposition filename is correctly prefixed with container name in both modes |
| `TestWebServerReportAndExportAssets` | Asserts embedded SPA dashboard assets (`dashboard.html`) include comprehensive cluster text report builder, container inspection report builder, client-side JSON exports (`report_type: 'cluster_telemetry'`, `report_type: 'container_inspection'`), NDJSON export action, `.fill-table-wrap` full-height layout, and widened 250px dropdown menu (`pkg/web`) | **PASS**: Embedded web dashboard contains all report formats, rich telemetry fields, and layout classes |
| `TestWebServerExportComprehensiveData` | Tests `/api/v1/export` cluster and single-container REST endpoints returning comprehensive telemetry data (aggregate system totals, health status, memory RSS/cache, cumulative network/disk I/O bytes, mount/network metadata, generated run command and compose specs) and validates 404 on missing container (`pkg/web`) | **PASS**: Correctly formats, serializes, and serves complete cluster and container telemetry snapshots in JSON format |
| `TestWebBridgeContainerLogs` | End-to-end integration test validating log streaming from mock container connector through `WebBridge` (`web_bridge_test.go`) | **PASS**: Reliably streams container log lines through `WebBridge` to HTTP clients |
| `TestWebServerExportSanitizesSecrets` | Security audit test verifying JSON exports (`/api/v1/export`), container details, and stream snapshots mask 100% of secrets and passwords (`pkg/web`) | **PASS**: Cleartext passwords never exported; all credentials masked with `•••••••••••• [masked]` |
| `TestWebServerSessionIdleTimeoutConfiguration` | Tests web session idle timeout configuration (default 30m, custom duration), reports via `/api/v1/auth/status`, and verifies session expiration/eviction (`pkg/web`) | **PASS**: Default 1800s applied, custom duration reported, and expired sessions correctly evicted |
| `TestWebServerSessionStoreIdleSlidingWindow` | Tests sliding-window session refresh behavior, ensuring active sessions remain valid while idle sessions expire and evict (`pkg/web`) | **PASS**: Active session prolonged during usage; expired after inactivity window |
| `TestWebServerStreamSessionExpiration` | Tests SSE stream connection termination when authenticated session cookie expires due to idle timeout (`pkg/web`) | **PASS**: Live stream detects expired session via `IsSessionValid` and cleanly closes connection |
| `TestWebDashboardEnvAndLogFilterFeatures` | Asserts embedded SPA dashboard assets (`dashboard.html`) include `envClearBtn`, `envFilterModeBtn`, `logFilterModeBtn`, alpha sorting with `localeCompare`, and client-side idle tracking (`pkg/web`) | **PASS**: Embedded web dashboard contains all new filter controls, modes, and idle timer logic |
| `TestWebDashboardJavaScriptFilterAndSort` | Headless Node.js unit tests executing the exact dashboard JavaScript algorithms for env alpha sorting, include/exclude env filtering, clearing, and include/exclude log filtering (`pkg/web`) | **PASS**: 100% of frontend sorting, filtering, and toggle algorithms pass automated assertions |
| `TestWebBridgeSessionTimeoutOption` | Tests `WebBridge` initialization with default (30m), custom (15m), and disabled (-1 / 0) session idle timeout options (`web_bridge_test.go`) | **PASS**: Accurately propagates `WebOptions.SessionTimeout` to underlying server instance |
...
| `TestExecShellNilCursor` | Tests `ExecShell` invocation when grid cursor is nil | **PASS**: Safely returns nil `MenuFn` without panics |
| `TestExecShellCommandSecurityAndScreenClean` | Validates in-container shell launcher command construction, verifying clean shell invocation, strict absence of escape sequence injection (printf/clear) or /etc/passwd and eval tokens to safeguard unprivileged service accounts | **PASS**: Clean shell discovery verified; printf, clear, /etc/passwd, nologin, eval strictly prohibited |
| `TestExecShellFallbackHierarchy` | Tests fallback shell execution sequence when primary or secondary shell commands fail | **PASS**: Attempts fallbacks in deterministic order and terminates gracefully without panics |
| `TestExecShellResidualEventDraining` | Tests draining of residual keystrokes (`exit<Enter>`, `Ctrl+D`) from `uiEvents` when returning from interactive shell | **PASS**: Residual events fully drained so next user keypress triggers immediately |
...
| `TestGridShortcutESingleViewProcess` | Tests pressing `E` on the main container grid directly navigating into Tab 5: Env/Proc (`SingleViewProcess`) and cleanly exiting back to the main grid on `q` (`grid.go`) | **PASS**: Direct shortcut dispatches `SingleViewProcess` and returns cleanly to main grid |
...
| `TestPrintHelp` | Tests CLI `--help` output formatting including `--session-timeout` flag | **PASS**: Help text contains all flags, options, and commands |
