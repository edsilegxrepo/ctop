...
func generateTestCertificates(t *testing.T, dir string) (string, string, string) {
...
func TestGlobalTLSConfig(t *testing.T) {
	tempDir := t.TempDir()
	caPath, certPath, keyPath := generateTestCertificates(t, tempDir)

	origConfig := getGlobalTLSConfig()
	defer SetGlobalTLSConfig(origConfig)
	...
	retrieved := getGlobalTLSConfig()
	if !retrieved.Verify || retrieved.CA != caPath || retrieved.Cert != certPath || retrieved.Key != keyPath {
		t.Fatalf("retrieved TLSConfig %+v does not match expected %+v", retrieved, cfg)
	}

	client, err := newDockerClient("tcp://127.0.0.1:2376")
	if err != nil {
		t.Fatalf("failed to instantiate client with TLSConfig: %v", err)
	}
	if client == nil {
